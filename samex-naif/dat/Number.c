...
MOD_INIT(Number) {
    ...
    GFN_add = makeGeneric(2, 2, GFN_NONE, stringFromUtf8(-1, "add"));
    datImmortalize(GFN_add);

    GFN_div = makeGeneric(2, 2, GFN_NONE, stringFromUtf8(-1, "div"));
    datImmortalize(GFN_div);

    GFN_divEu = makeGeneric(2, 2, GFN_NONE, stringFromUtf8(-1, "divEu"));
    datImmortalize(GFN_divEu);

    GFN_mod = makeGeneric(2, 2, GFN_NONE, stringFromUtf8(-1, "mod"));
    datImmortalize(GFN_mod);

    GFN_modEu = makeGeneric(2, 2, GFN_NONE, stringFromUtf8(-1, "modEu"));
    datImmortalize(GFN_modEu);

    GFN_mul = makeGeneric(2, 2, GFN_NONE, stringFromUtf8(-1, "mul"));
    ...
    GFN_sub = makeGeneric(2, 2, GFN_NONE, stringFromUtf8(-1, "sub"));

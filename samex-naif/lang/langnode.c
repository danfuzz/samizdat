...
zvalue makeClassDef(zvalue name, zvalue attributes, zvalue methods) {
    ...
    zvalue keys = METH_CALL_old(keyList, attribMap);
    ...
    zvalue instanceMethodTable = makeCall(SYMS(new),
        METH_APPLY(listFrom1(LITS(SymbolTable)), cat,
            METH_CALL_old(valueList, instanceMethods)));
...
zvalue makeFullClosure(zvalue baseData) {
    ...
    if (     (yieldNode == NULL)
        ...
        if (isExpression(lastStat)) {
            statements = METH_CALL_old(sliceExclusive, statements, intFromZint(0));
...
zvalue makeImport(zvalue baseData) {
    ...
    if (select != NULL) {
        ...
        if (valEq(select, SYM(CH_STAR))) {
            ...
            data = METH_CALL_old(del, data, SYM(select));
        }
    ...
    switch (recordEvalType(node)) {
        ...
        case EVAL_importModuleSelection: {
            ...
            if (select != NULL) {
                ...
                select = METH_CALL_old(keyList, exports);
...
zvalue withoutIntermediates(zvalue node) {
    return METH_CALL_old(del, node, SYM(box), SYM(interpolate), SYM(lvalue));
}

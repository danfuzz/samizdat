...
zvalue makeApply(zvalue function, zvalue values) {
    ...
    return recordFrom2(RECNAME_apply, SYM_function, function, SYM_values, values);
}
...
zvalue makeAssignmentIfPossible(zvalue target, zvalue value) {
    ...
    if (get(target, SYM_lvalue) == NULL) {
        ...
        return recordFrom2(RECNAME_store,
...
zvalue makeBasicClosure(zvalue table) {
    return makeRecord(RECNAME_closure,
        METH_CALL(cat,
...
zvalue makeCall(zvalue function, zvalue values) {
    ...
    return recordFrom2(RECNAME_call, SYM_function, function, SYM_values, values);
}
...
zvalue makeExport(zvalue node) {
    return recordFrom1(RECNAME_export, SYM_value, node);
}

// Documented in spec.
zvalue makeExportSelection(zvalue names) {
    return recordFrom1(RECNAME_exportSelection, SYM_select, names);
}
...
zvalue makeImport(zvalue baseData) {
    ...
    if (select != NULL) {
        ...
        return makeRecord(RECNAME_importModuleSelection, data);
    }
    ...
    if (get(data, SYM_format) != NULL) {
        ...
        return makeRecord(RECNAME_importResource, data);
    }

    // It's a whole-module import.
    return makeRecord(RECNAME_importModule, data);
}
...
zvalue makeInterpolate(zvalue node) {
    return recordFrom3(RECNAME_fetch,
        SYM_target,      node,
...
zvalue makeLiteral(zvalue value) {
    return recordFrom1(RECNAME_literal, SYM_value, value);
}
...
zvalue makeMaybe(zvalue value) {
    return recordFrom1(RECNAME_maybe, SYM_value, value);
}
...
zvalue makeVarRef(zvalue name) {
    return recordFrom1(RECNAME_varRef, SYM_name, name);
}

// Documented in spec.
zvalue makeVarDef(zvalue name, zvalue value) {
    return recordFrom2(RECNAME_varDef, SYM_name, name, SYM_value, value);
}

// Documented in spec.
zvalue makeVarDefMutable(zvalue name, zvalue value) {
    return recordFrom2(RECNAME_varDefMutable, SYM_name, name, SYM_value, value);
}

// Documented in spec.
zvalue makeVarFetch(zvalue name) {
    return recordFrom1(RECNAME_fetch, SYM_target, makeVarRef(name));
}
...
zvalue makeVarFetchLvalue(zvalue name) {
    ...
    // `lvalue`.
    return recordFrom2(RECNAME_fetch,

...
## Parses a `fn` definition statement or generic function binding. The
## "interesting" result of this rule is a `@closure` node, which is wrapped
## either in a `@topDeclaration` (regular function definition) or a `@call`
## node (generic function binding).
##
## The `@closure` translation is along these lines:
##
## ```
